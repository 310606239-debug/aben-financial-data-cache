...
- CSI 100 / 中证100：100 个证券，来自中证指数官方成份股文件。
- CSI 200 / 中证200：200 个证券，来自中证指数官方成份股文件。
...
- CSI 2000 / 中证2000：2000 个证券，来自中证指数官方成份股文件。
- CSI All Share / 中证全指：全部当前样本证券，来自中证指数官方成份股文件。
...
标准化代码采用市场后缀格式，例如 `AAPL`、`600519.SS`、`0700.HK`、
`920000.BJ`。北交所代码必须保留 `.BJ`，不能回退成深圳市场的 `.SZ`。
...
北交所证券会以正确的 `.BJ` 代码保留在股票池和缺口报告中。当前 yfinance 对北交所
新旧代码均可能返回无数据；在接入独立北交所数据源前，这些证券应明确报告为缺失，不能
误标为深交所证券或假装已经更新。
...
python -m scripts.update_cache --shard-index 0 --shard-count 48
```
